...
def add_to_cart(product_id: str, current_user=Depends(get_current_user)):
    ...
    if cart_item:
        # Increment quantity
        db["cart"].update_one({"_id": cart_item["_id"]}, {"$inc": {"quantity": 1}})
    else:
        # First time add
        db["cart"].insert_one(
...
def update_quantity(
    product_id: str, quantity: int, current_user=Depends(get_current_user)
):
    user_id = str(current_user["_id"])
    ...
    result = db["cart"].update_one(
        {"user_id": user_id, "product_id": product_id}, {"$set": {"quantity": quantity}}
    )
...
def clear_cart(current_user=Depends(get_current_user)):
    ...
    return {
        ...
        "deleted_items": result.deleted_count,
    }
