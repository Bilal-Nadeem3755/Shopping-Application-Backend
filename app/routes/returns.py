...
import os
from app.services.order_service import serialize_mongo
load_dotenv()
...
def create_return_request(
    ...
    if existing_return:
        raise HTTPException(
            status_code=400,
            detail="Return already requested"
        )
...
def get_all_returns(
    ...
    returns = list(db["returns"].find())

    return serialize_mongo(returns)